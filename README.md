...
- hybrid local retrieval with weighted fields, conversation-aware reranking, and focused passage selection
...
For each question, the browser retrieves at most five relevant notes, extracts up to 1,400 focused characters per note, and sends only those passages to the Ollama process on the same device. `NOTION_CONTENT_CHAR_LIMIT` controls how much page text remains available for this local passage search. Up to four recent turns help resolve references in follow-up questions, but previous answers are explicitly excluded as factual evidence. Every generated claim requires an inline citation and is checked again against the cited passage before it is displayed.
...
NOTION_CONTENT_CHAR_LIMIT=12000
...
| Knowledge Q&A | loaded graph + local Ollama | hybrid sparse retrieval, focused passages, Qwen generation, claim verification, citations, and extractive fallback |
